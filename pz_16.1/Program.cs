    ...
    internal class Program
    {
        ...
        static void GenerationMap()
        {
            ...
            while (enemies > 0) // Добавление врагов
            {
                ...
                if (map[x, y] == '_')     // Если ячейка на этих координатах пуста ,
                                          // то в данную ячейку ставится символ врага ('E') и количество врагов уменьшается на 1
                {
                    map[x, y] = 'E';
                    enemies--;
                }
        ...
        static void Move()
        {
            ...
            while (true)
            {
                ...
                for (int i = 0; i < lastAction.Count; i++)  // Вывод
                {
        ...
        static void Savegame() // Сохранение
        {
            ...
            using (StreamWriter writer = new StreamWriter(path)) // Запись в него параметров
            {
                ...
                for (int i = 0; i < mapSize; i++) // Запись карты
                {
                    ...
                    {
                        if (map[i, j] == 'P')    //  Если элемент равен символу 'P', то он заменяется на символ '_',
                                                 //  перед записью в файл
                        {       //  С целью скрыть позицию игрока в сохранении,
                                //  чтобы не было возможности изменить его позицию напрямую в файле.
                            map[i, j] = '_';
                        }
